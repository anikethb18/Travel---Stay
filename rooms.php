...
<body class="bg-light">
                        ...
                        <div class="collapse navbar-collapse flex-column align-items-stretch mt-2" id="filterDropdown">
                            ...
                            <div class="border bg-light p-3 rounded mb-3">
                                ...
                                while ($row = mysqli_fetch_assoc($facilities_q)) {
                                    echo <<<facilities
                                    <div class="mb-2">
                                        <input type="checkbox" onclick="fetch_rooms()" name="facilities" value="$row[id]" class="form-check-input shadow-none me-1" id="facility_$row[id]">
                            ...
                            <div class="border bg-light p-3 rounded mb-3">
                                <h5 class="d-flex align-items-center justify-content-between mb-3" style="font-size: 18px;">
                                    <span>CITY</span>
                                    <button id="city_btn" onclick="city_clear()" class="btn shadow-none btn-sm text-secondary d-none">Reset</button>
                                </h5>
                                <?php
                                $city_q = "SELECT DISTINCT `location` FROM `rooms` ORDER BY `location`";
                                $city_res = mysqli_query($con, $city_q);
                                while ($row = mysqli_fetch_assoc($city_res)) {
                                    echo <<<city
                                    <div class="mb-2">
                                        <input type="checkbox" onclick="fetch_rooms()" name="city" value="$row[location]" class="form-check-input shadow-none me-1" id="city_$row[location]">
                                        <label class="form-check-label" for="city_$row[location]">$row[location]</label>
                                    </div>
                                    city;
                                }
                                ?>
    ...
    <script>
        ...
        function fetch_rooms() {
            ...
            let get_cities = document.querySelectorAll('[name="city"]:checked');
            if (get_cities.length > 0) {
                get_cities.forEach((city) => {
                    city_list.cities.push(city.value);
                });
                city_btn.classList.remove('d-none');
            } else {
                city_btn.classList.add('d-none');
            }
            city_list = JSON.stringify(city_list);

            let xhr = new XMLHttpRequest();
            xhr.open("GET", "ajax/rooms.php?fetch_rooms&chk_avail=" + chk_avail + "&guests=" + guests + "&facility_list=" + facility_list + "&city_list=" + city_list, true);
