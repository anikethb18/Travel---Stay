<?php 
  ...
  if(isset($_POST['add_room']))
  {
    ...
    $q1 = "INSERT INTO `rooms` (`name`, `area`, `location`, `address`, `price`, `quantity`, `adult`, `children`, `description`, `travel_id`, `travel_agency`) VALUES (?,?,?,?,?,?,?,?,?,?,?)";
    $values = [$frm_data['name'],$frm_data['area'],$frm_data['location'],$frm_data['address'],$frm_data['price'],$frm_data['quantity'],$frm_data['adult'],$frm_data['children'],$frm_data['desc'],$user_id,$_SESSION['agencyName']];
    
    if(insert($q1,$values,'siissiiisis')){
      $flag = 1;
    }
  ...
  {
    $res = select("SELECT * FROM `rooms` WHERE `removed`=? and travel_id = $user_id",[0],'i');
    $i=1;
  ...
  if(isset($_POST['get_room']))
  {
    ...
    $res1 = select("SELECT * FROM `rooms` WHERE `id`=? and travel_id = $user_id",[$frm_data['get_room']],'i');
  ...
  if(isset($_POST['edit_room']))
  {
    ...
    $q1 = "UPDATE `rooms` SET `name`=?,`area`=?,`location`=?,`address`=?,`price`=?,`quantity`=?,
      `adult`=?,`children`=?,`description`=? WHERE `id`=?";
    $values = [$frm_data['name'],$frm_data['area'],$frm_data['location'],$frm_data['address'],$frm_data['price'],$frm_data['quantity'],$frm_data['adult'],$frm_data['children'],$frm_data['desc'],$frm_data['room_id']];
    
    if(update($q1,$values,'sissiiisi')){
      $flag = 1;
    }
