...
if (isset($_POST['add_flight'])) {
    ...
    $values = [$frm_data['flight_name'], $frm_data['departure_city'], $frm_data['arrival_city'], $frm_data['departure_time'], $frm_data['arrival_time'], $user_id, $_SESSION['agencyName'], $frm_data['status']];
...
if (isset($_POST['get_all_flights'])) {
    $res = select("SELECT * FROM `flights` WHERE `removed`=? and travel_id = $user_id", [0], 'i');
    $i = 1;
    $data = "";
...
if (isset($_POST['get_flight'])) {
    $frm_data = filteration($_POST);
    $res = select("SELECT * FROM `flights` WHERE `flight_id`=? and travel_id = $user_id", [$frm_data['get_flight']], 'i');
