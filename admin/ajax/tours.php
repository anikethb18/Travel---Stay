...
if (isset($_POST['add_tour'])) {
    ...
    $q = "INSERT INTO `tour_packages`(`name`, `destination`, `duration`, `price`, `description`, `travel_id`, `travel_agency`, `status`) VALUES (?,?,?,?,?,?,?,?)";
    $values = [$frm_data['name'], $frm_data['destination'], $frm_data['duration'], $frm_data['price'], $frm_data['desc'], $frm_data['travel_id'], $frm_data['travel_agency'], $frm_data['status']];
...
if (isset($_POST['get_all_tours'])) {
    $res = select("SELECT * FROM `tour_packages` WHERE `removed`=?", [0], 'i');
    $i = 1;
    $data = "";
...
if (isset($_POST['get_tour'])) {
    $frm_data = filteration($_POST);
    $res = select("SELECT * FROM `tour_packages` WHERE `package_id`=?", [$frm_data['get_tour']], 'i');
...
if (isset($_POST['edit_tour'])) {
    ...
    $values = [$frm_data['name'], $frm_data['destination'], $frm_data['duration'], $frm_data['price'], $frm_data['desc'], $frm_data['travel_id'], $frm_data['travel_agency'], $frm_data['status'], $frm_data['tour_id']];
