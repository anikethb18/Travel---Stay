...
if (isset($_GET['fetch_rooms'])) {
    ...
    // city filter data decode
    $city_list = json_decode($_GET['city_list'], true);
    ...
    // query for room cards with guests and city filter
    $room_where = "WHERE `adult`>=? AND `children`>=? AND `status`=? AND `removed`=?";
    $values = [$adults, $children, 1, 0];
    $type = 'iiii';

    if (count($city_list['cities']) > 0) {
        $city_placeholders = str_repeat('?,', count($city_list['cities']) - 1) . '?';
        $room_where .= " AND `location` IN ($city_placeholders)";
        $values = array_merge($values, $city_list['cities']);
        $type .= str_repeat('s', count($city_list['cities']));
    }

    $room_res = select("SELECT * FROM `rooms` $room_where", $values, $type);
    ...
    while ($room_data = mysqli_fetch_assoc($room_res)) {
        ...
        if ($chk_avail['checkin'] != '' && $chk_avail['checkout'] != '') {
            ...
            $tb_values = ['booked', $room_data['id'], $chk_avail['checkin'], $chk_avail['checkout']];
            ...
            <div class='card mb-4 border-0 shadow'>
                <div class='row g-0 p-3 align-items-center'>
                    ...
                    <div class='col-md-5 px-lg-3 px-md-3 px-0'>
                        ...
                        <h6 class='mb-2'><i class='bi bi-geo-alt-fill'></i> $room_data[location]</h6>
